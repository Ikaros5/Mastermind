...
/**
 * Ventana que se encarga de gestionar una partida de dificultad estándar
 */
public class ventanaPartidaEstándar extends JFrame implements ActionListener{
	...
	public ventanaPartidaEstándar(int id) {
		this.idP = id;
		...
		lblPuntuacin.setBounds(227, 28, 118, 23);
		lblPuntuacin.setForeground(new Color(0, 0, 0));
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(477, 91, 274, 195);
		
		JLabel label_2 = new JLabel("-");
		label_2.setFont(new Font("FreeSerif", Font.BOLD, 20));
		label_2.setBounds(346, 28, 65, 23);
		...
		panel_colores.setBounds(33, 519, 516, 72);
		...
		for (int i = 0; i < colores.length; i++) {
            ...
            if (i == 1 || i == 2) botones_colores[i].setForeground(Color.BLACK);
            else botones_colores[i].setForeground(Color.WHITE);
		...
		panel_casillas.setBounds(32, 404, 516, 72);
		panel_casillas.setLayout(new GridLayout(1, 0, 0, 0));
		
		botones_casillas = new JButton[colores.length];
        for (int i = 0; i < colores.length; i++) {
        ...
        panel_combinacion.setBounds(357, 304, 397, 63);
		...
		lblCasillas.setBounds(33, 377, 60, 15);
		...
		tglbtnAyuda.setBounds(28, 322, 83, 29);
		...
		Intentos.setBounds(33, 91, 432, 195);
		...
		lblRol.setFont(new Font("FreeSerif", Font.BOLD, 20));
		lblRol.setForeground(new Color(0, 0, 0));
		
		JLabel label_3 = new JLabel("-");
		label_3.setFont(new Font("FreeSerif", Font.BOLD, 20));
        ...
        btnComprueba.setFont(new Font("FreeSerif", Font.BOLD, 25));
		btnComprueba.setBounds(572, 389, 182, 202);
		    ...
		    public void actionPerformed(ActionEvent e) {
		    	...
		    	if (CtrlPresentacion.getRol(idP).equals("CodeBreaker")) {
		    		...
		    		if (counter == 2 && !enter) {
		    			...
		    			CtrlPresentacion.actualizarRankingEstandar(id);
		...
		lblCombinacinSecreta.setBounds(180, 298, 153, 15);
		...
		panel.setBounds(44, 22, 20, 0);
		
		JPanel panel_rol = new JPanel();
		panel_rol.setBounds(497, 22, 243, 29);
		...
		gl_panel_rol.setHorizontalGroup(
			gl_panel_rol.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_rol.createSequentialGroup()
					.addComponent(lblRol, GroupLayout.PREFERRED_SIZE, 45, GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(ComponentPlacement.RELATED)
					.addComponent(label_3, GroupLayout.PREFERRED_SIZE, 121, GroupLayout.PREFERRED_SIZE)
					.addGap(85))
		);
			...
			gl_panel_rol.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel_rol.createSequentialGroup()
					.addGroup(gl_panel_rol.createParallelGroup(Alignment.LEADING)
						.addComponent(lblRol, GroupLayout.PREFERRED_SIZE, 29, GroupLayout.PREFERRED_SIZE)
            ...
            for (int i = 0; i < botones_casillas.length; i++) {
                if (e.getSource() == botones_casillas[i]) {
                    ...
                    if (color_seleccionado != null) {
                        ...
                        if (botones_casillas[i].getBackground() == Color.YELLOW || botones_casillas[i].getBackground() == Color.GREEN) {
                        	botones_casillas[i].setForeground(Color.BLACK);
                        }
                        else botones_casillas[i].setForeground(Color.WHITE);
            ...
            for (int i = 0; i < botones_combinacion.length; i++) {
                if (e.getSource() == botones_combinacion[i]) {
                    ...
                    if (color_seleccionado != null) {
                        ...
                        if (botones_combinacion[i].getBackground() == Color.YELLOW || botones_combinacion[i].getBackground() == Color.GREEN) {
                        	botones_combinacion[i].setForeground(Color.BLACK);
                        }
                        else botones_combinacion[i].setForeground(Color.WHITE);
                        color_seleccionado = null;
                        casilla_seleccionada = -1;
                    }
                    return;
